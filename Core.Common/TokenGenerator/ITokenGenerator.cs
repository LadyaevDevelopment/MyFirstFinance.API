...
namespace Core.Common.TokenGenerator
{
	...
	{
		string Token(params string[] values);

		TokenValidationResult Validate(string code);

		public class Base : ITokenGenerator
		{
			...
			public string Token(params string[] values)
			{
			...
			public TokenValidationResult Validate(string code)
			{
				...
				try
				{
					...
					var result = new TokenValidationResult(true, values);
